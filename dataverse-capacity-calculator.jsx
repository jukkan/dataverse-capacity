...
const SKUS = [
  ...
  // D365 ERP Add-on (capacity values vary widely based on add-on, so not included in default or accrual - users can add manually as needed)
  {
    ...
    accrual: { db_gb: 10, file_gb: 10 },
    accrues_capacity: true,
    requires_base: undefined,
  },
  {
    id: "erp-sb3",
    name: "Tier 3 Sandbox Addon",
    family: "Dynamics365",
    product_group: "ERPAddon",
    license_type: "CapacityPack",
    eligible_for_default: true,
        ...
        if (count > 0) {
          ...
          if (sku.accrues_capacity) {
            ...
            if (sku.tenant_cap_db_gb !== undefined) {
              ...
              addDb = Math.min(
                addDb,
                Math.max(0, sku.tenant_cap_db_gb - existing),
              );
